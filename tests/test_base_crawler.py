...
class MockCrawler(BaseCrawler):
    ...
    Args:
        first_request_kwargs (Optional[dict[str, Any]]): The keyword arguments
            to pass to the first request. Default is None.
        **kwargs: Additional keyword arguments to pass
            to the `BaseCrawler` class.

    """

    def __init__(
        self, first_request_kwargs: Optional[dict[str, Any]] = None, **kwargs
    ) -> None:
    ...
    async def generate_requests(self) -> AsyncIterator[Request]:
        ...
        yield Request(
            ...
            callback=self.parse_with_return,
            **self.first_request_kwargs
        )
    ...
    async def parse_with_return(
        ...
        yield "test_item_2"
        yield Request(
            url="https://example.com/", callback=self.parse_with_error
        )
...
def test_get_http_client_kwargs() -> None:
    ...
    limits = Limits(
        max_connections=10, max_keepalive_connections=5, keepalive_expiry=60
    )
