...
async def test_from_httpx_response_with_iter_content() -> None:
    """Tests the `from_httpx_response` method of the `Response` class with an async iterator content."""
    chunks = [b"Hello, ", b"world!"]
    httpx_response = create_httpx_response(content=mock_iter_content(chunks))

    response = await Response.from_httpx_response(httpx_response, create_request())
    assert response.content == b"Hello, world!"
    assert response.text == "Hello, world!"


def test_get_json_data_with_valid_json() -> None:
    """Tests the `get_json_data` method of the `Response` class with valid JSON."""
    json_data = {"key": "value"}
...
def test_get_json_data_with_invalid_json() -> None:
    """Tests the `get_json_data` method of the `Response` class with invalid JSON."""
...
def test_selector(text: str) -> None:
    ...
    assert response.selector._text == expected_selector._text  # pylint: disable=W0212
    assert response.selector.type == expected_selector.type
